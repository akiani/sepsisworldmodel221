...
class SepsisEnvBayesian(gym.Env):
    ...
    def __init__(self, starting_state=None, verbose=False):
        ...
        self.state_model = keras.models.load_model(os.path.join(module_path, STATE_MODEL), 
                                                   compile=False, custom_objects={'MDN': mdn.MDN(NUM_FEATURES-2, 
                                                                                                 MIXTURES)})
        self.termination_model = tf.keras.models.load_model(os.path.join(module_path, TERMINATION_MODEL))
        self.outcome_model = tf.keras.models.load_model(os.path.join(module_path, OUTCOME_MODEL))
    ...
    def step(self, action):
        ...
        if termination_state == 'done':
            ...
            reward = reward[0][0]
            #print(reward)
        #take the condition from the 30 vector next state and incorportate that 
