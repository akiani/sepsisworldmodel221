...
class SepsisEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if termination_state == 'done':
            ...
            idx_sofa = features.index('sofa')
            idx_lactate = features.index('LACTATE')
            if self.s[idx_sofa] == next_state[0, idx_sofa] and next_state[0, idx_sofa] > 0:
                reward += -0.025
            reward += -0.125 * (next_state[0, idx_sofa] - self.s[idx_sofa])
            reward += -2 *  np.tanh((next_state[0, idx_lactate] - self.s[idx_lactate]).astype(np.float64))
            reward = reward[0][0]
            #print("current", self.s)
            #print("next",next_state)
            
            #print('delta sofa', next_state[0, idx_sofa] - self.state_0[idx_sofa])
            #print('delta lactate', next_state[0, idx_lactate] - self.state_0[idx_lactate])
            #val = self.state_0[idx]
            #reward = -0.1 * action
...
class SepsisEnvVariational(gym.Env):
    ...
    def step(self, action):
        ...
        #find the condition from the 30 vector next_state and then incorporate thatinto the  reward  function
        print(next_state)
